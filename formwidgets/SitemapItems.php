<?php namespace Utopigs\Sitemap\FormWidgets;

use Request;
use Utopigs\Sitemap\Classes\DefinitionItem as SitemapItem;
...
class SitemapItems extends FormWidgetBase
{
    ...
    public $referenceRequiredMessage = 'utopigs.sitemap::lang.item.reference_required';

    public $urlRequiredMessage = 'utopigs.sitemap::lang.item.url_required';

    public $cmsPageRequiredMessage = 'utopigs.sitemap::lang.item.cms_page_required';
    ...
    public function prepareVars()
    {
        ...
        $widgetConfig = $this->makeConfig('~/plugins/utopigs/sitemap/classes/definitionitem/fields.yaml');
     ...
     * @param \Utopigs\Pages\Classes\SitemapItem $item Specifies the sitemap item
     * @return string 
     */
    protected function getReferenceDescription($item)
    {
        ...
        if (isset($this->typeInfoCache[$item->type])) {
            ...
            $result = trans('utopigs.sitemap::lang.item.unknown_type');
        }
    ...
    protected function findReferenceName($search, $typeOptionList)
    {
        ...
        if (!strlen($result)) {
            $result = trans('utopigs.sitemap::lang.item.unnamed');
        }
    ...
    protected function getSitemapItemTitle($itemInfo)
    {
        if (is_array($itemInfo)) {
            if (!array_key_exists('title', $itemInfo) || !strlen($itemInfo['title'])) {
                return trans('utopigs.sitemap::lang.item.unnamed');
            }

            return $itemInfo['title'];
        }

        return strlen($itemInfo) ? $itemInfo : trans('utopigs.sitemap::lang.item.unnamed');
    }
}
